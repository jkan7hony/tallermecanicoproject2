		...
		<div class="row justify-content-center">
			<div class="col-12 col-lg-9 col-xl-8">
				<div class="text-center mb-4">
					...
					<h2 class="entity-title">Crear Vehículo</h2>
					<p class="entity-subtitle mb-0">Asocia un vehículo a un cliente registrado.</p>
				</div>

				<div class="card entity-card">
					<div class="card-body p-4 p-md-5">
						<form action="{{ route('vehiculos.store') }}" method="POST" class="entity-form">
							@csrf
							...
							<div class="row g-3">
								<div class="col-12">
									...
									<select name="cliente_id" class="form-select @error('cliente_id') is-invalid @enderror">
										...
										@foreach ($clientes as $cliente)
											<option value="{{ $cliente->id }}" @selected(old('cliente_id') == $cliente->id)>{{ $cliente->rut }} - {{ $cliente->nombre }} {{ $cliente->apellido }}</option>
										@endforeach
									</select>
								...
								<div class="col-md-6">
									...
									<input type="text" name="patente" value="{{ old('patente') }}" class="form-control @error('patente') is-invalid @enderror" placeholder="ABC123" inputmode="numeric" pattern="\d{6}" maxlength="6" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,6)">
								...
								<div class="col-md-6">
									...
									<input type="number" name="anio" value="{{ old('anio') }}" class="form-control @error('anio') is-invalid @enderror" placeholder="2024" min="1900" max="{{ date('Y') }}">
								...
								<div class="col-md-6">
									...
									<input type="text" name="modelo" value="{{ old('modelo') }}" class="form-control @error('modelo') is-invalid @enderror" placeholder="Hilux">
							...
							<div class="entity-actions d-flex flex-column flex-sm-row gap-3 justify-content-sm-between align-items-sm-center mt-4">
								...
								<button type="submit" class="btn btn-success px-5 py-2 fw-bold rounded-pill">GUARDAR VEHÍCULO</button>
							</div>
						</form>
					</div>
				</div>
